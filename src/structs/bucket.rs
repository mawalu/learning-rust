...
impl Kbuckets {
	pub fn new() -> Kbuckets {
		Kbuckets { buckets: vec!(Bucket::new(HashId::new([255; 20]))) }
	}
...
impl Bucket {
	...
	pub fn insert(&mut self, node: Node) -> Result<(), BucketError> {
		...
		if self.upper_boundary < node.node_id {
			return Err(BucketError::new("NodeID is not within the buckets boundary".to_string()))
		}
	...
	pub fn questionables(&self) -> Vec<&Node> {
		let mut questionable = self.nodes
			.iter()
...
mod tests {
	...
	fn test_list_questionables() {
		...
		old_node.last_seen = old_node.last_seen.checked_sub_signed(Duration::minutes(16)).unwrap();
		older_node.last_seen = older_node.last_seen.checked_sub_signed(Duration::minutes(20)).unwrap();
	...
	fn test_find_bucket() {
		...
		assert_eq!(buckets.find(HashId::new([10; 20])).unwrap().upper_boundary, split_boundary);
		assert_eq!(buckets.find(HashId::new([200; 20])).unwrap().upper_boundary, HashId::new([0xFF; 20]));
	}
}